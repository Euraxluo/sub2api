...
- `landing/`：Claude 风格首页插件。Admin Tools 内调参预览，渲染 HTML 后写入站点设置 `home_content`（不改上游 CSP/静态路由）。

## 首页落地页（仅 custom_tools）

1. 模板：`landing/index.html`
2. 参数与渲染：`landing/params.ts`、`landing/render.ts`
3. 管理 UI：`landing/LandingHomeTool.vue`（Admin Tools →「首页落地页」）
4. 调试：左侧改参数 / 用 query URL 导入导出；右侧 `srcdoc` 预览
5. 生效：一键「写入 HTML 到 home_content」（v-html 模式）。同域 iframe URL 方案需要改上游 CSP，不在插件边界内。
